...
): { allowed: boolean; limit: number; remaining: number; resetTime: number; retryAfter?: number } {
  const now = Date.now();
    ...
    return new NextResponse(
      ...
      }),
      {
        ...
        headers: {
          ...
          'X-RateLimit-Reset': Math.floor(hourlyCheck.resetTime / 1000).toString(),
        },
      }
    );
  }
    ...
    return new NextResponse(
      ...
      }),
      {
        ...
        headers: {
          ...
          'X-RateLimit-Reset': Math.floor(burstCheck.resetTime / 1000).toString(),
        },
      }
    );
  }
      ...
      return new NextResponse(
        ...
        }),
        {
          ...
          headers: {
            ...
            'X-RateLimit-Reset': Math.floor(sessionCheck.resetTime / 1000).toString(),
          },
        }
      );
    }
...
export function addRateLimitHeaders(
  ...
  if (hourlyEntry) {
    response.headers.set('X-RateLimit-Limit-Hourly', RATE_LIMITS.USER_HOURLY.limit.toString());
    response.headers.set('X-RateLimit-Remaining-Hourly', Math.max(0, RATE_LIMITS.USER_HOURLY.limit - hourlyEntry.count).toString());
    response.headers.set('X-RateLimit-Reset-Hourly', Math.floor(hourlyEntry.resetTime / 1000).toString());
  }
  
  if (burstEntry) {
    response.headers.set('X-RateLimit-Limit-Burst', RATE_LIMITS.USER_BURST.limit.toString());
    response.headers.set('X-RateLimit-Remaining-Burst', Math.max(0, RATE_LIMITS.USER_BURST.limit - burstEntry.count).toString());
    response.headers.set('X-RateLimit-Reset-Burst', Math.floor(burstEntry.resetTime / 1000).toString());
  }
